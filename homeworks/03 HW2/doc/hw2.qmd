...
We’ll implement the core pieces of a **deep neural network (DNN)** ourselves: its linear layers, ReLU, loss, and backpropagation ([Task @sec-task1]). Then we’ll rebuild the same idea in PyTorch (a python library), letting it take over the math and use it to revisit the HW1 gene‑expression regression task ([Task @sec-task2]).
...
*2.1.1* What is the purpose of the ReLU activation function?

*2.1.2* Is backpropagation **required** for training a neural network? Why or why not?

*2.1.3* What are the main components of the train loop? What is the purpose of each component? Does the order of these components matter? Why or why not?

*2.1.4* Explain the difference between gradient descent and back-propagation.

*2.1.5* How many parameters does your DNN have? (PyTorch version), how did you calculate/find it?
...
*2.2.1* For the DNN, plot the training & validation set loss curves for different hyper-parameters. How did changing each hyper-parameter affect the curves? Report the best performing DNN hyper-parameters and the final model structure.
...
*2.2.2* Discuss and compare the results of the LR model and the DNN (mention MSE, Pearson *r*, and number of parameters).
...
A: For the first parts of @sec-task1, you are allowed to use NumPy alone. For the other sections, you may use any library, but you cannot use any pre-built models. 
