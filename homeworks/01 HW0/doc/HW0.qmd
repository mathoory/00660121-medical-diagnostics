...
submission-date: 28.6.26 (Non-mandatory)
author:
  - name: Mattan Hoory
    acknowledgements: | 
...
While the tools listed here represent a recommended setup, especially for those new to machine learning workflows, you're free to use any environment you're comfortable with. The core of this course is not about mastering any one tool or code, but about understanding the principles and reasoning behind machine learning methods which are ideas that remain useful even as technologies change. 

HW0 is **not** graded or submitted. While submission is in pairs, all students are expected to complete it before the first tutorial. Expect ~30 min. The collab path is faster.
...
For the assignments in this course you will need to write code, while you are scored based on your written report, your code will also be submitted. You can write code in any environment you prefer, but we recommend using a Jupyter notebook in Python.

There are three recommended ways for running code in this course:

1. **Google Colab**  –  free cloud notebooks with optional GPU
2. **Kaggle Notebooks**  –  free cloud notebooks persistent storage and GPU quotas
3. **Locally on your PC**  –  Conda + VS Code (full control, offline access)

While running remotely (Google Colab or Kaggle) is easier to set up, running code locally gives you more control and flexibility.
...
GPUs are specialized hardware designed to make computation faster, particularly for tasks involving large amounts of data and parallel processing. They are especially useful in machine learning and deep learning, where they can significantly speed up training times for models.
...
In any case where a GPU is recommended, it will be clearly stated in the assignment. And in such a case we will try to provide suitable options for running the code on a GPU.
...
* Either open a **New notebook** or **Upload** an existing notebook.
...
* On the left panel, click on **Create**
...
* By default, if no run is made for 40 minutes, the session will be disconnected and all variables and files will be deleted. You can change this by choosing a **Persistence** that suits your needs.
...
If either box fails, the fallback is Colab and the forum.
