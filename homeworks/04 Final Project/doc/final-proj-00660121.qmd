...
[Basecalling](https://en.wikipedia.org/wiki/Base_calling) is the inverse problem: recover the sequence from the squiggle. In @fig-squiggle you can read the answer off the picture because the bases are printed underneath. Your model gets only the blue line the includes measurement noise. Production basecallers, such as Oxford Nanopore's [Dorado](https://nanoporetech.com/platform/technology/basecalling), do this with a Transformer.
...
**The input.** One read is the 480 current samples of @fig-squiggle. Write it $s$:

$$ s \;=\; [\,s_1,\, s_2,\, \ldots,\, s_{480}\,] \;\in\; \mathbb{R}^{480} $$

**Step 1, tokenization.** Individual samples are the wrong unit to reason with. There are 60
...
Do that for all 60 steps and stack the results as rows. That matrix is the model's working
...
One caution about the word. In Tutorial 4 a token was a word and the **embedding** was a
lookup in a learned table, one row per word in a fixed vocabulary. There is no vocabulary
here and no lookup: the current values are already numbers, and the convolution *computes*
each token from them. Same role in the pipeline, different mechanism.

**Step 2, the stack.** Each encoder block takes 60 rows and returns 60 rows of the same
width, so $X$ keeps its shape all the way through. What changes is the contents of each row,
which is where self-attention comes in below.

**Step 3, the head and the answer.** A `Linear` layer maps each row to 4 numbers, one per
base:
...
to 1, and the largest one is the base we call. Taking the first two rows as an example:

$$
\begin{array}{c|cccc|c}
 & \texttt{A} & \texttt{C} & \texttt{G} & \texttt{T} & \\\hline
\text{scores, step } 1 & 2.7 & -0.4 & 0.1 & -1.2 & \\
\text{probabilities}   & 0.87 & 0.04 & 0.06 & 0.02 & \Rightarrow\; \texttt{A} \\\hline
\text{scores, step } 2 & -0.9 & 0.2 & 3.1 & 0.0 & \\
\text{probabilities}   & 0.02 & 0.05 & 0.90 & 0.03 & \Rightarrow\; \texttt{G}
\end{array}
$$

Reading the winner off all 60 rows gives the called sequence $\hat{y}$, which is what we
...
**What the loss is computed on.** Not on the letters. Accuracy compares $\hat{y}$ with $y$,
but you cannot train on it, because taking the largest entry throws away how confident the
model was and gives no gradient. The loss instead reads $O$ directly: at each of the 60
...
Task 1 did the two-choice version of exactly this, where `sigmoid` produced one probability
and `BCEWithLogitsLoss` measured how wrong it was. Here `softmax` and `nn.CrossEntropyLoss`
do the four-choice version. As with `BCEWithLogitsLoss`, the softmax is applied inside the
...
The notebook gives you these as classes. Their constructors take exactly the sizes above:
...
*B.* **Run the CNN** (Section 2). It is written out for you, since you built one in Task 1. Train it and read the two orientation columns separately. `evaluate` reports overall accuracy and the two orientations separately.
