...
*C.* Train the model using different architectures and any hyper-parameters you find relevant. Remember, you are limited to the GPU time quota. The term architecture here refers to the model layers and their paramters. For example: number of convolutional layers, their kernel sizes, and the number of filters in each layer. You can also experiment with different learning rates, batch sizes, and optimizers.
...
[Nanopore sequencing](https://en.wikipedia.org/wiki/Nanopore_sequencing) reads DNA by pulling a single strand through a tiny protein pore while measuring the ionic current across it. The few bases inside the pore at any moment set the current level, so a read comes out as a noisy one-dimensional time series called a *"squiggle"*. [Basecalling](https://en.wikipedia.org/wiki/Base_calling) is the inverse problem: turn the squiggle back into the DNA sequence `ACGT`. Modern basecallers do this with the Transformer, the same architecture that powers language models.

This task is built as an investigation, and `task-2.ipynb` is written to be worked through **in order**. You will look at the data, build the model you already know from Task 1, find where it fails, work out why, and only then reach for a Transformer. The sections below give you the technical background you need. The notebook carries the argument.

#### Background

**Input and output.** One read is a stretch of DNA that was pulled through the pore. We fix the read at $T = 60$ bases, and the pore emits $D = 8$ current samples per base, so the raw signal is a vector of $60 \times 8 = 480$ numbers:

$$
s \;=\; [\,s_1,\, s_2,\, \ldots,\, s_{480}\,] \;\in\; \mathbb{R}^{480}
\qquad\text{for example}\qquad
s = [\,0.31,\, -1.20,\, 0.80,\, \ldots,\, 0.14\,]
$$

The first 8 numbers belong to base 1, the next 8 to base 2, and so on. What we want back is one letter per base:

$$
y \;=\; [\,y_1,\, y_2,\, \ldots,\, y_{60}\,], \qquad y_i \in \{\texttt{A}, \texttt{C}, \texttt{G}, \texttt{T}\}
$$

The model does not output letters directly. Exactly as in Task 1, it outputs **numbers** and we read the answer off them. Task 1 produced one number per sequence (bind or not). Here we need one of **four** choices at each of 60 positions, so the model outputs a grid of $60 \times 4$ scores, and the largest score in a row is that base's call:
...
Row 1's largest entry is $2.7$ under `A`, so base 1 is called `A`. The loss function compares this grid with the true bases, and the notebook provides it, so you do not need to derive it.

**Tokenization.** The 480 raw numbers are not a useful input on their own, because the model should reason about *bases*, not about individual current samples. So the first layer groups each base's 8 samples into a single vector of length $d$, called a **token**:

$$
\underbrace{s \in \mathbb{R}^{480}}_{\text{480 current samples}}
\quad\longrightarrow\quad
\underbrace{X \in \mathbb{R}^{T \times d}}_{\text{60 tokens, one per base}}
$$

so row $i$ of $X$, written $x_i \in \mathbb{R}^{d}$, is everything the model knows about base $i$. You choose $d$ (it is the `d_model` setting). This is done with a convolution of width 8 and stride 8, which is the Task 1 trick again: a filter that slides along the signal, here stepping a whole base at a time.

Note the shapes: the input to the stack is $X \in \mathbb{R}^{T \times d}$ and the output is $O \in \mathbb{R}^{T \times 4}$. The number of rows never changes. Every layer in between transforms the $T$ tokens and hands back $T$ tokens.

**Self-attention.** This is the mechanism from Tutorial 4. Each token $x_i$ produces a query, a key and a value, and the attention weight from token $i$ to token $j$ compares $i$'s query with $j$'s key, scaled by the key width $d_k$:
...
The sum runs over **every** token $j$, so base $i$ can pull in information from anywhere in the read, near or far.

**Building the model.** To build the architecture you use the layers below, and you assemble them in the same way you assembled the CNN in Task 1: a list of layers, applied in order, each one handing its output to the next.

$$
\text{signal} \;\to\; [\text{Tokenizer}] \;\to\; +[\text{Positional encoding}] \;\to\; N\times[\text{Encoder block}] \;\to\; [\text{Head}] \;\to\; \texttt{ACGT}
$$

- **Tokenizer** - the convolution described above. Turns the 480 signal samples into $T$ tokens of width `d_model`.
- **Positional encoding** - attention alone has no notion of order, so this adds a fixed position signature to each token. Just like in the tutorial, it is added to the tokens, not applied as a layer.
- **Encoder block** - the part that does the thinking. Inside it, self-attention lets every token look at every other token, and then a small network is applied to each token on its own. It also does several other things that keep a deep stack trainable, which is why real implementations of this block are longer than you might expect. All of it is written for you: you create the block with a size and use it, and nothing inside it is yours to change or to understand for this project. You stack `n_layers` of them.
- **Head** - a `Linear` layer that turns each token into its 4 scores, exactly like the final `Linear(..., 1)` that ended your CNN in Task 1, only with 4 outputs instead of 1.

Your architecture decisions are the sizes: the token width `d_model`, the number of attention heads `n_heads`, how many encoder blocks `n_layers`, and the width `d_ff` of the small network inside each block.

#### Instructions

Work through `task-2.ipynb` in order. Each step below corresponds to one section of the notebook.

*A.* Read the Background above and skim how [Dorado basecalls](https://nanoporetech.com/platform/technology/basecalling). Open `task-2/task-2.ipynb` and read the data generator in Section 1. It is the ground truth for everything that follows.

*B.* **Look at the data** (Section 1). Plot a forward read next to a reverse read and compare them, drawing a few different reads.

*C.* **Build the CNN** (Section 2, `TODO-1`). Start from the architecture you already know from Task 1 and train it. The `evaluate` function reports accuracy split by orientation, so read the two columns separately rather than the overall number.

*D.* **Diagnose** (Section 3). Run the provided plot and work out what it is telling you.

*E.* **Build the Transformer** (Section 4, `TODO-2` and `TODO-3`). Assemble it from the provided layers, pick your sizes, and train it on the *same* data.

*F.* **Look inside it** (Section 5). Run the provided attention plot on a reverse read.

*G.* **Ablation** (Section 6, `TODO-4`). Rebuild with `use_pe=False`, retrain, and compare.

*H.* **Harder data, bigger models** (Section 7, `TODO-5`). Sweep `noise` for a couple of model sizes and plot accuracy against noise, then train in the graded regime.

#### Questions

*Q10* Compare a forward read with a reverse read: what differs between them? Then report your CNN's accuracy on each of the two orientations, and use the plot from step *D* to explain what defeats it. What property of a convolutional architecture is responsible, and why would adding more channels to the CNN not help?

*Q11* Report your Transformer's accuracy on both orientations next to your CNN's. What changed, and what does that tell you about the difference between the two architectures?

*Q12* From the attention plot, where does the queried base look? Relate this to your answer in *Q10*.

*Q13* What happened to accuracy when you set `use_pe=False`? Explain using the fact that self-attention on its own is *permutation-invariant*, meaning it does not know the order of its tokens.

*Q14* Report your best model's architecture (`d_model`, `n_heads`, `n_layers`, `d_ff`, and total parameter count) and how you chose it. Include your accuracy-vs-noise plot for at least two model sizes, and describe the trade-off you found.

*Q15* In a short paragraph, relate self-attention here to its use in protein language models, which attend across amino-acid residues. Why is the ability to look across a whole sequence useful in biology?

#### Model performance (competitive factor)

Part of the grade rewards accuracy in a fixed, harder data regime we set for grading (`noise=0.4, hetero=0.6, dwell_jitter=0.15`), evaluated on held-out reads. In this regime the data is genuinely difficult, so model choices matter. Aim for a model that is both accurate and light enough to train within the free-GPU quota. Report your final held-out accuracy.

#### Compute for this task

Lighter than Task 1: the models are small and a full training run takes a few minutes on a free Colab or Kaggle GPU (see @sec-gpu). The catch is that you will do *many* runs here, since the experiments in step *E.* sweep both the data sliders and the model sizes. Plan which runs you actually need, and debug with `steps` cut to a few dozen before launching a full sweep.
...
├── src/                 # your code files
│   ├── task-1.ipynb     # Task 1: CNN for RNA-protein binding sites
│   ├── task-2.ipynb     # Task 2: Transformer for nanopore basecalling
