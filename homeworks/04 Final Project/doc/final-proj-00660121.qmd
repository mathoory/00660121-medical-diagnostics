...
*B.* **Run the CNN** (Section 2). It is written out for you. Train it, and read the two orientation columns rather than the overall number.
...
*C.* **Train the Transformer** (Section 3, `TODO-1`). The model is written out for you, as the CNN was. Choose your sizes, and train it on the same data. Note the parameter counts that `train` prints, since the two models are deliberately close in size.
...
*D.* **Choose a configuration** (Section 4, `TODO-2`). Vary the model sizes and the data settings, and keep the runs you need to justify a choice. Part of the grade rewards how well that choice performs: your submitted notebook is re-run on a fixed set of data settings that we do not publish, scored with `evaluate` so that every submission is measured on identical reads. Your configuration has to hold up in conditions you could not tune for, which is what the sweep is for. Save the model you settle on with the cell at the end of Section 4 and include the resulting `basecaller.pt` in your submission - that file is what the performance grade is computed from, so a model you cannot reload is a model we cannot score.
...
*F.* **Look inside it** (Section 6, `TODO-4`). Plot the attention maps on a reverse read and work out which head is doing which job. Then silence each head in turn and re-score, so you can set what the maps suggest against what the model actually loses. Whether attention weights count as an explanation at all is an open argument in the literature.

*Q15* Which attention head in your model is doing what, and what gave each one away? Relate this to your answer in *Q11*. Then set the maps against the ablation: report what each head costs the model on each orientation, and say whether a head's picture predicted its cost. Name any head where the two disagree, and account for the disagreement.
...
everything else alone, what would come out of the model, and why would the loss stop working?
