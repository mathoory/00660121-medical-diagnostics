...
*Q2* How can CNN filters capture sequence motifs? What is the purpose of the convolutional layers, and how do they differ from fully connected ones?
...
*E.* **Read a motif off the model.** Complete the readout in `task-1.ipynb` (Section 3.1, `TODO-3`) by assembling the provided helpers. Export a handful of filters, turn them into logos with [WebLogo](https://weblogo.threeplusone.com/create.cgi), and include two or three in your report. Expect some to come out flat: a filter that learned nothing is a result too.
...
*E.* **Ablation** (Section 5, `TODO-3`). An *ablation* removes one part of a working model and retrains without it, so that whatever performance is lost can be attributed to that part. Here the part is the positional encoding: rebuild with `use_pe=False`, keeping every other size fixed, and retrain. Sections 5 and 6 both work on a trained model of your choosing - say which one in your report.
...
Look again at what self-attention computes: $z_i = \sum_j a_{ij} v_j$. The new vector for
token $i$ is a weighted average of the values from *every* token, and $a_{ij}$ is the weight
on token $j$. So each of those numbers answers something concrete: **when the model computed
the new vector for token $i$, how much of it came from token $j$?**

Those weights come from a softmax over $j$, so row $i$ is a distribution over where token $i$
drew its information from: spread evenly it would be $1/60 \approx 0.017$ in every cell, and a
bright cell means token $i$ took most of its input from one place.

One read gives $60 \times 60$ of these numbers per head. Drawn as an image, with row $i$ down
the page, column $j$ across it and brightness for $a_{ij}$, it shows where all 60 tokens looked
at once. Three patterns are worth recognising on sight: a **main diagonal**, each token
attending to itself and its neighbours; a **vertical stripe**, every token attending to the
same one column; and an **anti-diagonal**, token $i$ attending to token $59-i$.

![The three shapes, drawn by hand rather than taken from a model. A trained head may show one of them, a mixture, or nothing recognisable at all.](assets/attention-shapes.png){#fig-shapes width=100% fig-pos="H"}
