...
*E.* **Read a motif off the model.** Complete the readout in `task-1.ipynb` (TODO-3) by assembling the provided helpers. Export a handful of filters, turn them into logos with [WebLogo](https://weblogo.berkeley.edu/logo.cgi), and include two or three in your report. Expect some to come out flat: a filter that learned nothing is a result too.
...
[Basecalling](https://en.wikipedia.org/wiki/Base_calling) is the inverse problem: recover the sequence from the squiggle. In @fig-squiggle you can read the answer off the picture because the bases are printed underneath. Production basecallers, such as Oxford Nanopore's [Dorado](https://nanoporetech.com/platform/technology/basecalling), do this with a Transformer. Your model gets only the blue dots, and attempts to reconstruct the sequence. In other words, it learns (approximates) a function:
...
Lighter than Task 1. A 3,000-step run takes about a minute on a free Colab GPU (see
@sec-gpu), and the models are small enough that the CNN in Section 2 will finish on a CPU if
you have to. The sweep in step *D* is the part that wants a GPU, because it is many runs
rather than one long one.

Worth knowing where that minute goes, because it changes how you pick a model. The reads are
generated by NumPy on the **CPU**, and on a free Colab instance that generation sets the pace
rather than the GPU. The CNN and the Transformer take about the same wall-clock despite the
Transformer doing far more arithmetic. So a bigger model is close to free here, and choosing a
small one to save time mostly saves nothing. What costs you is the *number* of runs. Decide
what each run is testing before you launch it, and debug with `steps` cut to a few dozen.

#### The data

You will not download anything for this task. You will generate your reads using a simulation.

In Task 1 you were handed files of RNA sequences, with labels somebody else had
established in a lab. Here you start from the opposite end. You will create a random strand of DNA,
and simulate the current a pore would measure as that strand passed through it. These simulated measurements are your input, and the original strand is your label. Because you created the strand in the first place you already know every base of the answer. People who build real basecallers train them this way too, because the physics of the pore is understood well enough to create synthetic data.
...
Two values control the noise the reads have, and you will use them to test how your model performs under different conditions:
...
One step gives one level. A sliding window of 3 over 6 bases only fits four times, and our simulator wraps the first two windows around the end of the strand so that a 60-base read gives exactly 60 levels. This is also why a base cannot be identified on its own: the level at each step depends on the two bases before it as well. Each level is held for 8
...
and forward captures do not.

#### From signal to sequence

The model turns one read into one sequence. It is worth walking the whole path once, because
every shape in it comes back later.

**The input.** One read is 480 current samples, of which @fig-squiggle shows the first twelve steps. Write it $s$:

$$ s \;=\; [\,s_1,\, s_2,\, \ldots,\, s_{480}\,] \;\in\; \mathbb{R}^{480} $$

**Tokenization.** Individual samples are the wrong unit to reason with. There are 60
ratchet steps and 8 samples belong to each, so the first layer takes one step's 8 samples and
turns them into a single vector of width $d$, called a **token**:

$$ \underbrace{[\,s_1,\ldots,s_8\,]}_{\text{step 1}} \;\longrightarrow\; x_1 \in \mathbb{R}^{d}
\qquad
\underbrace{[\,s_9,\ldots,s_{16}\,]}_{\text{step 2}} \;\longrightarrow\; x_2 \in \mathbb{R}^{d} $$

Do that for all 60 steps, a count we write $T = 60$, and stack the results as rows. That matrix is the model's working
representation of the read:

$$ X \;=\; \begin{bmatrix} x_1 \\ x_2 \\ \vdots \\ x_{60} \end{bmatrix} \;\in\; \mathbb{R}^{60 \times d} $$

$d$ is yours to choose and is called `d_model`. The layer that does this is a convolution of
width 8 and stride 8, the Task 1 trick again, stepping one ratchet step at a time.

In Tutorial 4 a token was a word and the embedding was a lookup in a learned table, one row per word in a fixed vocabulary. Now, the convolution computes the tokens. Same role in the pipeline, different mechanism.

**Model Output.** After applying attention, to get an prediction, a `Linear` layer maps each token to 4 numbers, one per
base in a matrix similiar to one-hot encoding we call $O$:

$$ O \;\in\; \mathbb{R}^{60 \times 4} $$

Row $i$ holds the model's four scores for step $i$. These are raw scores, not probabilities:
...
\text{scores, step } 1 & 2.7 & -0.4 & 0.1 & -1.2 & \\
...
loss, so your model's head must output the raw scores $O$ with no softmax after it.
...
You assemble the architecture the same way you assembled the CNN in Task 1, as a list of layers we provide for you applied in order:
...
- **Positional encoding**: attention alone has no notion of order, so this adds a fixed position signature to each token. Note what it does to the tokens: it *adds* to them rather than replacing them, so the width stays `d_model`.
- **Encoder block**: self-attention followed by a small per-token network, plus several things that keep a deep stack trainable. You stack `n_layers` of them.
...
```
Tokenizer(d_model)                        PositionalEncoding(d_model)
EncoderBlock(d_model, n_heads, d_ff)      nn.Linear(d_model, 4)
```
...
*B.* **Run the CNN** (Section 2). It is written out for you, since you built one in Task 1. Train it, and read the two orientation columns rather than the overall number.
...
*C.* **Build the Transformer** (Section 3, `TODO-1` and `TODO-2`). Assemble it from the provided layers, choose your sizes, and train it on the same data. Note the parameter counts that `train` prints, since the two models are deliberately close in size.
...
*D.* **Choose an architecture** (Section 4, `TODO-3`). Vary the model sizes and the data settings, and keep the runs you need to justify a choice. Then train your chosen model in the graded regime you are ranked on, `noise=0.5, hetero=0.6` (see *Model performance*, below).
...
*E.* **Ablation** (Section 5, `TODO-4`). Predict what removing the positional encoding will do to each orientation, then rebuild with `use_pe=False`, keeping every other size fixed, and retrain.
...
*F.* **Look inside it** (Section 6). Run the attention plot on a reverse capture and work out which head is doing which job. Before you do, one caution: a bright pattern shows where attention weight *went*, not that the model used what it found there. Information also travels through the residual connections, and the prediction is read off the last block rather than the one you are plotting. Whether attention weights count as an explanation at all is an open argument in the literature.
...
Part of the grade rewards accuracy in a fixed, harder regime (`noise=0.5, hetero=0.6`), scored with `evaluate` so that every submission is measured on identical reads. Report your final accuracy.
...
| Model performance (competitive factor, Task 2)                               | 15  |
